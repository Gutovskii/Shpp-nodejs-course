...
export class SpeciesController {
  ...
  async createSpecies(
    @Body(ValidationPipe) dto: CreateSpeciesDto,
    @UploadedFiles() images: Express.Multer.File[],
  ) {
    const speciesToCreate = await this._mapper.mapAsync(
      dto,
      CreateSpeciesDto,
      Species,
    );
  ...
  async updateSpecies(
    ...
    @Body(ValidationPipe) dto: UpdateSpeciesDto,
  ) {
    const exists = await this._speciesService.exists(id);
    if (!exists) throw new NotFoundException();
    const speciesToUpdate = await this._mapper.mapAsync(
      dto,
      UpdateSpeciesDto,
      Species,
    );
  ...
  async addRelations(
    @Param('id', ParseIntPipe) id: number,
    @Body(ValidationPipe) relations: SpeciesRelations,
  ) {
  ...
  async removeRelations(
    ...
    const speciesToUpdate = await this._speciesService.findOne(id);
    if (!speciesToUpdate) throw new NotFoundException();
    return this._speciesService.removeRelations(speciesToUpdate, {
      ...relations,
    });
  }
}
