...
export class RelationsService {
  constructor(private _repoWrapper: RepositoryWrapper) {}

  async addRelations<T extends EntityInterface>(
    entity: T,
      ...
      if (entity.hasOwnProperty(connectedEntityName)) {
        ...
        if (typeof relations[connectedEntityName] === 'object') {
          (relations[connectedEntityName] as number[]).map(async (id) => {
            const entityToConnect = this._repoWrapper[
              generalEntityName as keyof RepositoryWrapper
          ...
          entitiesToConnect = entitiesToConnect.filter((etc) =>
            (entity[connectedEntityName] as EntityInterface[]).every(
              (e) => e.id !== etc.id,
            ),
          );
          (entity[connectedEntityName] as EntityInterface[]).push(
            ...entitiesToConnect,
          );
        } else if (typeof relations[connectedEntityName] === 'number') {
          const id = relations[connectedEntityName] as number;
          const entityToConnect = await this._repoWrapper[
            generalEntityName as keyof RepositoryWrapper
          ].findOne({ where: { id }, loadEagerRelations: false });
  ...
  async removeRelations<T extends EntityInterface>(
    entity: T,
    relations: Partial<Record<string, number[] | number>>,
  ): Promise<T> {
    for (const connectedEntityName in relations) {
      if (entity.hasOwnProperty(connectedEntityName)) {
        ...
        if (typeof relations[connectedEntityName] === 'object') {
          ...
          (relations[connectedEntityName] as number[]).map((id) => {
            const entityToDisconnect = this._repoWrapper[
              generalEntityName as keyof RepositoryWrapper
            ].findOne({ where: { id }, loadEagerRelations: false });
          ...
          if (entitiesToDisconnect.some((etd) => !etd))
            throw new NotFoundException();
          entity[connectedEntityName] = (
            entity[connectedEntityName] as EntityInterface[]
          ).filter((e) => entitiesToDisconnect.every((etd) => etd.id !== e.id));
        } else if (typeof relations[connectedEntityName] === 'number') {
          const id = relations[connectedEntityName] as number;
          const entityToDisconnect = await this._repoWrapper[
            generalEntityName as keyof RepositoryWrapper
          ].findOne({ where: { id }, loadEagerRelations: false });
