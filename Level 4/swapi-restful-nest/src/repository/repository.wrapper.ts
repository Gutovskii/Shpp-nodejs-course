...
export class RepositoryWrapper {
    ...
    get films(): FilmsRepository {
        if (!this._filmsRepository)
            return new FilmsRepository(this._ds);
        return this._filmsRepository;
    }

    get people(): PeopleRepository {
        if (!this._peopleRepository)
            return new PeopleRepository(this._ds);
        return this._peopleRepository;
    }

    get planets(): PlanetsRepository {
        if (!this._planetsRepository)
            return new PlanetsRepository(this._ds);
        return this._planetsRepository;
    }

    get species(): SpeciesRepository {
        if (!this._speciesRepository)
            return new SpeciesRepository(this._ds);
        return this._speciesRepository;
    }

    get starships(): StarshipsRepository {
        if (!this._starshipsRepository)
            return new StarshipsRepository(this._ds);
        return this._starshipsRepository;
    }

    get vehicles(): VehiclesRepository {
        if (!this._vehiclesRepository)
            return new VehiclesRepository(this._ds);
        return this._vehiclesRepository;
    }

    get fileImages(): FileImagesRepository {
        if (!this._fileImagesRepository)
            return new FileImagesRepository(this._ds);
    ...
    get users(): UsersRepository {
        if (!this._usersRepository)
            return new UsersRepository(this._ds);
        return this._usersRepository;
    }

    get roles(): RolesRepository {
        if (!this._rolesRepository)
            return new RolesRepository(this._ds);
