...
import {
  SwapiEntity,
  SwapiPageResponse,
} from 'src/common/interfaces/swapi-page-response.interface';
...
export class SwapiSeederService implements SeederInterface {
  ...
  private async seedEntities(
    entityName: string,
    entityType: ModelIdentifier<EntityInterface>,
    dtoType: ModelIdentifier,
  ) {
    this._logger.log(`Seeding started: [${entityName}]`);
    const addedResults: Promise<EntityInterface[]>[] = [];
    let page = (
      await firstValueFrom(
        this._httpService.get<SwapiPageResponse>(entityName, {
          data: { page: 1 },
        }),
      )
    ).data;
    for (let pageNum = 1; ; pageNum++) {
      ...
      page.results.map((result) => {
        const swapiEntity = _.mapKeys(result, (value, key) =>
          _.camelCase(key),
        ) as SwapiEntity;
      ...
      addedResults.push(
        this._repoWrapper[entityName as keyof RepositoryWrapper].createMany(
          entitiesToSeed,
        ),
      );
      ...
      page = (
        await firstValueFrom(
          this._httpService.get<SwapiPageResponse>(page.next),
        )
      ).data;
    }
    ...
    for (let pageNum = 1; ; pageNum++) {
      page.results.map((result) => {
        ...
        for (const entityName of this.entityNames) {
          if (
            swapiEntity.hasOwnProperty(entityName) &&
            swapiEntity[entityName]
          ) {
            if (typeof swapiEntity[entityName] === 'object') {
              relationObject[entityName] = (
                swapiEntity[entityName] as string[]
              ).map((url) => this.getIdFromUrl(url));
            } else if (typeof swapiEntity[entityName] === 'string') {
              relationObject[entityName] = this.getIdFromUrl(
                swapiEntity[entityName] as string,
              );
            }
          }
        ...
        addedRelations.push(
          new Promise(async (resolve) => {
            const entityToUpdate = await this._repoWrapper[
              entityName as keyof RepositoryWrapper
            ].findOne({ where: { id: entityId } });
            const updatedEntity = (await this._relationsService.addRelations(
              entityToUpdate,
              relationObject,
            )) as any;
            await this._repoWrapper[entityName as keyof RepositoryWrapper].save(
              updatedEntity,
            );
            resolve();
          }),
  ...
  async truncate(queryRunner: QueryRunner) {
    ...
    this._logger.log(
      'Truncating swapi entities has been finished successfully!',
    );
  }
