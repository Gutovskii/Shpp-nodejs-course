...
export const getItems = async (req: Request, res: Response) => {
    ...
    try {
        const tasks: ITask[] = await Task.find({ forUser: req.session.userId })
...
export const changeItem = async (req: Request, res: Response) => {
    try {
        ...
        const task: ITask | null = await Task.findOneAndUpdate({ _id: req.body._id }, req.body, {
             runValidators: true
        })
        if (!task) {
...
export const deleteItem = async (req: Request, res: Response) => {
    try {
        ...
        const task: ITask | null = await Task.findOneAndDelete({ _id: req.body._id })
        if (!task) {
            return res.status(404).json({ error: 'Bad Request' })
        }
